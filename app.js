...
for(let i =0; i <= alienInvaders.length -1; i++) {
  if(alienInvadersTakeDown[i] > (squares.length - (width -1))) {
    resultDisplay.textContent = 'Game Over';
    clearInterval(invaderId);
  }
}
...
// declare a win
if(alienTakenDown.length === alienInvaders.length) {
...
function shoot(e) {
  ...
  function moveLaser() {
    ...
    if(currentLaserIndex < width) {
      clearInterval(laserId);
      setTimout(() => squares[currentLaserIndex].classList.remove('laser'), 100)
    }
  }
